...
function tweetyReadDate(val, includeTime) {
 ...
 */
function tweetyAddCommonValidationRules(objectType, id) {
    Validation.addAllThese([
        ['validate-nospace', Zikula.__('No spaces', 'module_Tweetie'), function(val, elem) {
            var valStr;
            valStr = new String(val);
            return (valStr.indexOf(' ') === -1);
        }],
        ['validate-htmlcolour', Zikula.__('Please select a valid html colour code.', 'module_Tweetie'), function(val, elem) {
            var valStr;
            valStr = new String(val);
